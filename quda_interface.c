...
// reorder spinor to QUDA format
void reorder_spinor_toQuda_oo( double* sp, QudaPrecision precision, int doublet, double* sp2 ) {
  double startTime = gettime();
...
// reorder spinor from QUDA format
void reorder_spinor_fromQuda_oo( double* sp, QudaPrecision precision, int doublet, double* sp2 ) {
  double startTime = gettime();
...
void convert_eo_to_quda_oo( spinor* spinorIn, spinor* spinorOdd ) {
  double startTime = gettime();
#ifdef OMP
#pragma omp parallel
  {
...
#pragma omp for
  ...
  for(x = 0; x < LX; x++) {
	...
	for(t = 0; t < T; t++) {
	  ix = g_ipt[t][x][y][z];
	  i = g_lexic2eosub[ ix ];
	  j = x + LX*y + LY*LX*z + LZ*LY*LX*t;
	  if((t+x+y+z+g_proc_coords[3]*LZ+g_proc_coords[2]*LY 
	      + g_proc_coords[0]*T+g_proc_coords[1]*LX)%2 != 0) {
	       memcpy((spinorIn+j/2), (spinorOdd+i), sizeof(spinor));
	  }
	}
      }
    }
  ...
  double diffTime = endTime - startTime;
  if(g_proc_id == 0)
    printf("# QUDA: time spent in convert_eo_to_quda_oo: %f secs\n", diffTime);
}


void convert_quda_oo_to_eo( spinor* spinorOut, spinor* spinorOdd ) {
  double startTime = gettime();
#ifdef OMP
#pragma omp parallel
  {
...
#pragma omp for
  ...
  for(x = 0; x < LX; x++) {
	...
	for(t = 0; t < T; t++) {
	  ...
	  if((t+x+y+z+g_proc_coords[3]*LZ+g_proc_coords[2]*LY 
	      + g_proc_coords[0]*T+g_proc_coords[1]*LX)%2 != 0) {
	       memcpy((spinorOdd+i), (spinorOut+j/2), sizeof(spinor));
	  }
	}
      }
    }
  ...
  double diffTime = endTime - startTime;
  if(g_proc_id == 0)
    printf("# QUDA: time spent in convert_quda_oo_to_eo: %f secs\n", diffTime);
}
...
//  convert_eo_to_lexic(solver_field[1], Even_new, Odd_new);
  ...
  convert_eo_to_quda_oo(spinorIn, Odd);
  ...
  convert_quda_oo_to_eo(spinorOut, Odd_new);
  //convert_lexic_to_eo(Even_new, Odd_new, solver_field[1]);
